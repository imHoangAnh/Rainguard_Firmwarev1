...
bool sensor_mpu6050_read(mpu6050_data_t *data)
{
    ...
    data->gyro_x = ((float)gyro_x_raw / gyro_sensitivity) - sensor_state.gyro_offset_x;
    data->gyro_y = ((float)gyro_y_raw / gyro_sensitivity) - sensor_state.gyro_offset_y;
    data->gyro_z = ((float)gyro_z_raw / gyro_sensitivity) - sensor_state.gyro_offset_z;
...
bool sensor_mpu6050_read_raw(float *accel_x, float *accel_y, float *accel_z,
    ...
    if (accel_x) *accel_x = (float)accel_x_raw / accel_sensitivity;
    if (accel_y) *accel_y = (float)accel_y_raw / accel_sensitivity;
    if (accel_z) *accel_z = (float)accel_z_raw / accel_sensitivity;
    if (gyro_x) *gyro_x = (float)gyro_x_raw / gyro_sensitivity - sensor_state.gyro_offset_x;
    if (gyro_y) *gyro_y = (float)gyro_y_raw / gyro_sensitivity - sensor_state.gyro_offset_y;
    if (gyro_z) *gyro_z = (float)gyro_z_raw / gyro_sensitivity - sensor_state.gyro_offset_z;
...
bool sensor_mpu6050_self_test(void) {
    ...
    if (accel_mag < 0.5f || accel_mag > 1.5f) {
        ESP_LOGW(TAG, "Self-test warning: accel magnitude %.2fg (expected ~1g)", accel_mag);
    }

    ESP_LOGI(TAG, "Self-test passed: accel_mag=%.2fg, gyro=[%.1f, %.1f, %.1f] deg/s",
