...
// Configuration from network_config.h (manual configuration file)
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base,
  ...
  if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
    ...
    if (s_retry_num < MAX_RETRY) {
      esp_wifi_connect();
      s_retry_num++;
      ESP_LOGI(TAG, "Retry to connect to the AP (attempt %d/%d)", s_retry_num,
               MAX_RETRY);
    } else {
...
bool app_network_upload_image(camera_fb_t *fb) {
  ...
  body_size += fb->len;
  char part2[256]; // Increased from 128 to 256 to avoid format-truncation
                   // warning
